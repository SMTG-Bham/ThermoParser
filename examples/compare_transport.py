...
quantities = 'conductivity seebeck electronic_thermal_conductivity'

if run_boltztrap:
    tp.data.run.boltztrap(zero_weighted=False, run=False)
...
#adata = tp.data.load.amset(afile, quantities)
#adata = tp.data.resolve.resolve(adata, quantities, direction=direction)
